  ...
  return (
    <div>
      <div className="relative flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md gap-4">
        ...
        <div className="px-5 mb-5 pt-1 flex flex-col items-center">
          <h5 className="text-xl text-hotPink font-medium">{project.title}</h5>

          <div className="mb-5 mt-3 flex items-center justify-between text-left">
            <p className="text-base  text-left">{project.description}</p>
          </div>
          <a href={project.ctaLink} target="_blank" className="rounded-md text-almostWhite bg-hotPink px-5 py-3 text-center text-sm font-medium hover:bg-opacity-70 w-full transition-all duration-300 hover:shadow-md">
            {project.ctaText}
          </a>
        </div>
      </div>
